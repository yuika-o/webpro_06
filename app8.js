...
// メッセージを受け取って保存するエンドポイント
app.post("/qre/message", (req, res) => {
...
// アンケート投票を更新するエンドポイント
app.put("/qre/survey", (req, res) => {
  const { answer } = req.body;  // リクエストから回答を取得

  // 有効な選択肢かチェック
  if (surveyResults[answer] !== undefined) {
    surveyResults[answer]++;  // 投票数を増やす
    res.status(200).send({ success: true, message: "投票が成功しました" });
  } else {
    res.status(400).send({ success: false, message: "無効な選択肢です" });
  }
});
